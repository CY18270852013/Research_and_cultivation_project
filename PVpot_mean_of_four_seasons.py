# -*- coding: utf-8 -*-
"""
Created on Wed Apr 24 16:22:50 2024
...
for i, (season, months) in enumerate(seasons.items()):
    # 提取指定季节的风速数据
    selected_data = pv.sel(year=years[(years >= 2007) & (years <= 2022)],
                                   month=np.isin(data['month'].values, months),
                                   lon=lon_range, lat=lat_range)
    # 计算季节平均风速
    average_pv = selected_data.mean(dim=['year', 'month'])
    ...
    ax.set_title(f'PVpot mean ({season} {2007}-{2022})')
    ...
    # 提取南海区域的风速数据
    south_china_sea_p = average_pv.sel(lon=slice(south_china_sea_extent[0], south_china_sea_extent[1]),
                                                  lat=slice(south_china_sea_extent[2], south_china_sea_extent[3]))
    
    # 绘制南海区域的风速分布
    im_inset = ax_inset.pcolormesh(south_china_sea_p['lon'].values, south_china_sea_p['lat'].values,
                                   south_china_sea_p.values, cmap='jet', transform=ccrs.PlateCarree(), antialiased=True)
...
# 绘制colorbar
plt.colorbar(im_inset, label='PVpot', cax=cax)
